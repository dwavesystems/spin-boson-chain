Finite chain subject to yz-noise
================================
...
.. math ::
    ...
    :label: finite_chain_yz_noise_H_A

with :math:`L` being the number of qubits:

.. math ::
    L=2,
    :label: finite_chain_yz_noise_L
...
.. math ::
    ...
    :label: finite_chain_yz_noise_h_x_r

.. math ::
    h_{z;r}\left(t\right)=\frac{B\left(t\right)}{2}h_{r},
    :label: finite_chain_yz_noise_h_z_r

.. math ::
    A\left(t\right)=2\left\{ 1-s\left(t\right)\right\} ,
    :label: finite_chain_yz_noise_A

.. math ::
    B\left(t\right)=2s\left(t\right),
    :label: finite_chain_yz_noise_B

.. math ::
    s\left(t\right)=\frac{t}{t_{a}},
    :label: finite_chain_yz_noise_normalized_anneal_fraction

.. math ::
    h_{r}=\begin{cases}
    \frac{1}{5}, & \text{if }r=0,\\
    -\frac{1}{10}, & \text{if }r=1,
    \end{cases}
    :label: finite_chain_yz_noise_h_r
...
.. math ::
    ...
    :label: finite_chain_yz_noise_J_zz

.. math ::
    J_{0,1}=-\frac{9}{10},
    :label: finite_chain_yz_noise_J
...
.. math ::
    ...
    :label: finite_chain_yz_noise_H_B
...
.. math ::
    ...
    :label: finite_chain_yz_noise_H_AB
...
.. math ::
    ...
    \hat{Q}_{\nu;r},
    :label: finite_chain_yz_noise_mathcal_Q

with :math:`\mathcal{E}_{v;r}^{\left(\lambda\right)}\left(t\right)` being a
time-dependent energy scale:

.. math ::
    \mathcal{E}_{y;r}^{\left(\lambda\right)}\left(t\right)=\begin{cases}
    \frac{1}{4}\sqrt{A\left(t\right)}, & \text{if }r=0,\\
    0, & \text{if }r=1,
    \end{cases}
    :label: finite_chain_yz_noise_mathcal_E_y
...
.. math ::
    ...
    \end{cases}
    :label: finite_chain_yz_noise_mathcal_E_z
...
.. math ::
    ...
    :label: finite_chain_yz_noise_Q_v_r
...
.. math ::
    \lambda_{\nu;r;\epsilon}=
    \delta_{\epsilon,\epsilon_{\varsigma=0}}\sqrt{\eta_{\nu;r;\varsigma=0}},
    :label: finite_chain_yz_noise_lambda

.. math ::
    \eta_{y;0;0}=\frac{1}{5}; \quad \eta_{y;1;0}=0;
    :label: finite_chain_yz_noise_eta_y

.. math ::
    \eta_{z;0;0}=0; \quad \eta_{z;1;0}=\frac{4}{5};
    :label: finite_chain_yz_noise_eta_z

.. math ::
    \omega_{y ;\epsilon=\epsilon_{0}}=5;
    \quad\omega_{z ;\epsilon=\epsilon_{0}}=1.
    :label: finite_chain_yz_noise_peak_frequencies
...
.. math ::
    ...
    :label: finite_chain_yz_noise_rho_i
...
.. math ::
    ...
    :label: finite_chain_yz_noise_rho_i_B
...
.. math ::
    ...
    :label: finite_chain_yz_noise_rho_i_A
...
.. math ::
    ...
    :label: finite_chain_yz_noise_psi_i
...
.. math ::
    \hat{\sigma}_{\nu;r}\left|\pm\nu\right\rangle _{r}=
    \pm\left|\pm\nu\right\rangle _{r}.
    :label: finite_chain_yz_noise_defining_nu_eigenstates
...
Eq. :eq:`finite_chain_yz_noise_H_A`, and the spectral densities of noise at
...
.. math ::
    ...
    :label: finite_chain_yz_noise_introducing_spectral_densitities
...
.. math ::
    ...
    :label: finite_chain_yz_noise_Q_operator_in_heisenberg_picture
...
.. math ::
    ...
    \frac{e^{-\beta\hat{H}^{\left(B\right)}}}{\text{Tr}^{\left(B\right)}
    \left\{ e^{-\beta\hat{H}^{\left(B\right)}}\right\} },
    :label: finite_chain_yz_noise_initial_state_operator

with :math:`\beta=1/\left(k_{B}T\right)`:

.. math ::
    \beta = 1,
    :label: finite_chain_yz_noise_beta
...
.. math ::
    ...
    :label: finite_chain_yz_noise_A_v_r_T
...
.. math ::
    ...
    :label: finite_chain_yz_noise_A_v_r_0T

with :math:`A_{\nu;r;T=0;\varsigma}\left(\omega\right)` being the
:math:`\varsigma^{\text{th}}`-component of
:math:`A_{\nu;r;T=0}\left(\omega\right)`. In this example, each spectral density
of noise trivially has one component. From
Eqs. :eq:`finite_chain_yz_noise_Q_v_r`-:eq:`finite_chain_yz_noise_peak_frequencies`,
and
:eq:`finite_chain_yz_noise_introducing_spectral_densitities`-:eq:`finite_chain_yz_noise_initial_state_operator`,
...
.. math ::
    ...
    :label: finite_chain_yz_noise_A_v_r_0T_varsigma

We choose a model with discrete spectral densities of noise so that we can
...
2-qubit system subject to both :math:`y`- and :math:`z`-noise, where the
system's bath correlation time, or "memory", is larger than the total simulation
time.

In order to implement the discrete spectral densities of noise in ``Python``, we
...
.. math ::
    ...
    \right\},
    :label: finite_chain_yz_noise_A_v_r_0T_varsigma_rewritten
...
.. math ::
    ...
    :label: finite_chain_yz_noise_dirac_delta_sequence
...
.. math ::
    ...
    :label: finite_chain_yz_noise_IR_cutoff
	    
.. math ::	    
    \omega_{\varsigma}^{\left(\text{UV}\right)}
    =\omega_{\nu;\epsilon_{\varsigma}}+8w,
    :label: finite_chain_yz_noise_UV_cutoff

and
	    
.. math ::
    w=10^{-6}.
    :label: finite_chain_yz_noise_w_2
...
.. math ::
    ...
    :label: finite_chain_yz_noise_ev_of_sx
...
.. math ::
    ...
    \hat{\sigma}_{z;r}\right\} ,\quad\text{for }r\in\left\{ 0,1\right\};
    :label: finite_chain_yz_noise_ev_of_sz

.. math ::
    \left\langle \hat{\sigma}_{z;0}\left(t\right)
    \hat{\sigma}_{z;1}\left(t\right)\right\rangle =\text{Tr}^{\left(A\right)}
    \left\{ \hat{\rho}^{\left(A\right)}\left(t\right)\hat{\sigma}_{z;0}
    \hat{\sigma}_{z;1}\right\};
    :label: finite_chain_yz_noise_ev_of_szsz
...
.. math ::
    ...
    :label: finite_chain_yz_noise_ev_of_sysz

.. math ::
    \left\langle \hat{\sigma}_{x;0}\left(t\right)
    \hat{\sigma}_{z;1}\left(t\right)\right\rangle =
    \text{Tr}^{\left(A\right)}\left\{ \hat{\rho}^{\left(A\right)}\left(t\right)
    \hat{\sigma}_{x;0}\hat{\sigma}_{z;1}\right\};
    :label: finite_chain_yz_noise_ev_of_sxsz

.. math ::
    \left\langle \hat{\sigma}_{x;0}\left(t\right)
    \hat{\sigma}_{x;1}\left(t\right)\right\rangle =
    \text{Tr}^{\left(A\right)}\left\{ \hat{\rho}^{\left(A\right)}\left(t\right)
    \hat{\sigma}_{x;0}\hat{\sigma}_{x;1}\right\};
    :label: finite_chain_yz_noise_ev_of_sxsx
...
.. math ::
    ...
    :label: finite_chain_yz_noise_probabilities
...
.. math ::
    ...
    \left(t\right)\right\}.
    :label: finite_chain_yz_noise_ev_of_energy
...
Eqs. :eq:`finite_chain_yz_noise_ev_of_sx`-:eq:`finite_chain_yz_noise_ev_of_energy`
...
find the same code in the file `examples/finite-chain/yz-noise/example.py` of
the repository.

.. literalinclude:: ../examples/finite-chain/yz-noise/example.py
