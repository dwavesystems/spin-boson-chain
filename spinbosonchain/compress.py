...
class Params():
    ...
    one can perform singular value decomposition (SVD) truncations to compress
    efficiently the MPS's. In short, increasing and decreasing the parameters
    ...
    positive number. If ``rel_tol`` is set as such, then ``spinbosonchain`` will
    discard singular values smaller than ``rel_tol * s_max``, where ``s_max`` is
    the largest singular value after the initial truncation procedure has been
    performed.
