...
etas = {("z", 0, 0): 0.0, ("z", 1, 0): 4/5, ("z", 2, 0): 0.0}
mode_frequencies = {("z", 0): 1}
w = 1e-6
E_z = (0, 3/4, 0)

spectral_densities_0T = {"z": [0]*L}
coupling_energy_scales = {"z": [0]*L}
for noise_type in ("z",):
    for r in range(L):
        cmpnts = []
...
op_string_set_1 = ("upupup|", "downdowndown|")
op_string_set_2 = ["".join(str_seq)+"|"
                   for str_seq in itertools.product('zI', repeat=3)]
for op_string_1 in op_string_set_1:
    static_list = []
    sign = 1 if op_string_1 == "upupup|" else -1
...
filenames_vs_op_strings_map = \
    {quspin_output_dir + "/sx.csv": ["xII|", "IxI|", "IIx|"],
     quspin_output_dir + "/sz.csv": ["zII|", "IzI|", "IIz|"],
     quspin_output_dir + "/sz|sz.csv": ["zzI|", "Izz|"],
     quspin_output_dir + "/sy|sz.csv": ["yzI|", "Iyz|"],
     quspin_output_dir + "/multi-site-spin-op-1.csv": ["xzx|"],
     quspin_output_dir + "/multi-site-spin-op-2.csv": ["xxx|"],
     quspin_output_dir + "/spin-config-1.csv": ["upupup|"],
     quspin_output_dir + "/spin-config-2.csv": ["downdowndown|"],
...
sx_id_id_y_label = (r"$\left\langle\hat{\sigma}_{x;r=0}"
                    r"\left(t\right)\right\rangle$")
id_sx_id_y_label = (r"$\left\langle\hat{\sigma}_{x;r=1}"
                    r"\left(t\right)\right\rangle$")
id_id_sx_y_label = (r"$\left\langle\hat{\sigma}_{x;r=2}"
                    r"\left(t\right)\right\rangle$")
sz_id_id_y_label = (r"$\left\langle\hat{\sigma}_{z;r=0}"
                    r"\left(t\right)\right\rangle$")
id_sz_id_y_label = (r"$\left\langle\hat{\sigma}_{z;r=1}"
                    r"\left(t\right)\right\rangle$")
id_id_sz_y_label = (r"$\left\langle\hat{\sigma}_{z;r=2}"
                    r"\left(t\right)\right\rangle$")
sz_sz_id_y_label = (r"$\left\langle\hat{\sigma}_{z;r=0}\left(t\right)"
                    r"\hat{\sigma}_{z;r=1}\left(t\right)\right\rangle$")
id_sz_sz_y_label = (r"$\left\langle\hat{\sigma}_{z;r=1}\left(t\right)"
                    r"\hat{\sigma}_{z;r=2}\left(t\right)\right\rangle$")
sy_sz_id_y_label = (r"$\left\langle\hat{\sigma}_{y;r=0}\left(t\right)"
                    r"\hat{\sigma}_{z;r=1}\left(t\right)\right\rangle$")
id_sy_sz_y_label = (r"$\left\langle\hat{\sigma}_{y;r=1}\left(t\right)"
                    r"\hat{\sigma}_{z;r=2}\left(t\right)\right\rangle$")
sx_sz_sx_y_label = (r"$\left\langle\hat{\sigma}_{x;0}\left(t\right)"
                    r"\hat{\sigma}_{z;1}\left(t\right)"
                    r"\hat{\sigma}_{x;2}\left(t\right)\right\rangle$")
sx_sx_sx_y_label = (r"$\left\langle\hat{\sigma}_{x;0}\left(t\right)"
                    r"\hat{\sigma}_{x;1}\left(t\right)"
                    r"\hat{\sigma}_{x;2}\left(t\right)\right\rangle$")
...
base_plot_filenames_vs_plot_specs_map = \
    {"/sx|id|id.pdf": ("/sx.csv", 1, sx_id_id_y_label),
     "/id|sx|id.pdf": ("/sx.csv", 2, id_sx_id_y_label),
     "/id|id|sx.pdf": ("/sx.csv", 3, id_id_sx_y_label),
     "/sz|id|id.pdf": ("/sz.csv", 1, sz_id_id_y_label),
     "/id|sz|id.pdf": ("/sz.csv", 2, id_sz_id_y_label),
     "/id|id|sz.pdf": ("/sz.csv", 3, id_id_sz_y_label),
     "/sz|sz|id.pdf": ("/sz|sz.csv", 1, sz_sz_id_y_label),
     "/id|sz|sz.pdf": ("/sz|sz.csv", 2, id_sz_sz_y_label),
     "/sy|sz|id.pdf": ("/sy|sz.csv", 1, sy_sz_id_y_label),
     "/id|sy|sz.pdf": ("/sy|sz.csv", 2, id_sy_sz_y_label),
     "/sx|sz|sx.pdf": ("/multi-site-spin-op-1.csv", 1, sx_sz_sx_y_label),
     "/sx|sx|sx.pdf": ("/multi-site-spin-op-2.csv", 1, sx_sx_sx_y_label),
