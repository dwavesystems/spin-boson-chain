...
For infinite chains, we take the limit of :math:`N\to\infty`.
...
class Model():
    ...
    module :mod:`spinbosonchain.system`.
    ...
    **Suggestion to users:** If simulating an infinite chain that can be
    ...
    implemented in ``spinbosonchain`` using a single-site unit cell, rather than
    a redundant multi-site unit cell. For example, an infinite chain that is
    ...
    should be modelled in ``spinbosonchain`` using a single-site unit
    cell. Generally speaking, the algorithm used for the case of an infinite
    chain with a multi-site unit cell is numerically less stable than that of an
    infinite chain with a single-site unit cell.

    Parameters
    ----------
    z_fields : `array_like` (`float` | :class:`spinbosonchain.scalar.Scalar`, shape=(``L``,)) | `None`, optional
        ...
        are converted to trivial instances of 
        :class:`spinbosonchain.scalar.Scalar`. If ``z_fields`` is set to `None` 
        (i.e. the default value), then no longitudinal fields are applied to the
        spins.
    x_fields : `array_like` (`float` | :class:`spinbosonchain.scalar.Scalar`, shape=(``L``,)) | `None`, optional
        The transverse field energy scales for a unit cell of size ``L``. If
        ``x_fields`` is an array, then ``x_fields[r]`` is the field energy 
        scale for site ``r``. Note that the field energy scales can be either 
        time-dependent or independent. All time-independent field energy scales 
        are converted to trivial instances of 
        :class:`spinbosonchain.scalar.Scalar`. If ``x_fields`` is set to `None` 
        (i.e. the default value), then no transverse fields are applied to the 
        spins.
    zz_couplers : `array_like` (`float` | :class:`spinbosonchain.scalar.Scalar`, shape=(``L-1+int(is_infinite)``,)) | `None`, optional
        ...
        of :class:`spinbosonchain.scalar.Scalar`. If ``zz_couplers`` is set to 
        `None` (i.e. the default value), then no longitudinal couplers are 
    ...
    Attributes
    ----------
    z_fields : `array_like` (:class:`spinbosonchain.scalar.Scalar`, shape=(``L``,)), read-only
    ...
    x_fields : `array_like` (:class:`spinbosonchain.scalar.Scalar`, shape=(``L``,)), read-only
        ...
        field energy scales can be either time-dependent or independent. 
    zz_couplers : `array_like` (:class:`spinbosonchain.scalar.Scalar`, shape=(``L-1+int(is_infinite)``,)), read-only
        The longitudinal coupling energy scales for a unit cell of size ``L``.
